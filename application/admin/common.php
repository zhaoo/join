...
/**
 * 生成操作标签
 * @param array $operate 操作标签数组
 */
function showOperateLabel($operate = [])
{
    if(empty($operate)){
        return '';
    }
    ...
    foreach($operate as $key=>$vo){
        if(authCheck($vo['auth'])){
            $option .= ' <a href="' . $vo['href'] . '"><span class="label label-' . $vo['labelStyle'] . '">' . $key . '</span></a>';
        }
    }
    return $option;
}
...
function createQrcode($data)
{
    ...
    $outfile = ROOT_PATH.'public/upload/images/qrcode'.'/'.md5($data).".jpg";
    $returnUrl = '/upload/images/qrcode'.'/'.md5($data).'.jpg';
    $level = 'M';
    $size =4;
    $QRcode = new \QRcode();
    ob_start();
